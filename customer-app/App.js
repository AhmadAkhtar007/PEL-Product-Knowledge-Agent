...
export default function App() {
  ...
  // Registration flow
  const [regMode, setRegMode] = useState(null); // null | 'qr' | 'manual'
  const [regCategory, setRegCategory] = useState('Refrigerator');
  const [regModel, setRegModel] = useState('Glass Door Prism Series');
  ...
  const fetchAppliances = async () => {
    try {
      const res = await fetch(`${BACKEND_URL}/appliances`);
      if (res.ok) {
          ...
          if (block.trim()) {
            ...
            if (event === 'thinking') {
              ...
              try {
                ...
                if (actionData.action === 'dial' && actionData.number) {
                  Linking.openURL(`tel:${actionData.number}`).catch(err => {
                    console.log('Error opening dialer:', err);
                    Alert.alert('Dialer Error', 'Could not open the phone dialer.');
                  });
                } else if (actionData.action === 'open_app') {
                  Linking.canOpenURL('pelkhidmat://').then(supported => {
                    if (supported) {
                      Linking.openURL('pelkhidmat://');
                    } else {
                      Linking.openURL('https://play.google.com/store/apps/details?id=com.pel.pelKhidmatMarkaz&hl=en');
                    }
                  });
                } else if (actionData.action === 'open_whatsapp') {
                  Linking.openURL('whatsapp://send?phone=923111735111').catch(() => {
                    Alert.alert('WhatsApp Not Installed', 'Please install WhatsApp to contact support.');
                  });
                } else if (actionData.action === 'tool_result') {
  ...
  const triggerQrScan = () => {
    ...
    setTimeout(() => {
      setQrScanning(false);
      setRegCategory('Refrigerator');
      setRegModel('Glass Door Prism Series');
      setRegSerial('PEL-REF-' + Math.floor(100000 + Math.random() * 900000));
      setRegDate(new Date().toISOString().split('T')[0]);
      setRegMode('manual');
      Alert.alert('Scan Successful', 'Found appliance: Refrigerator (Model: Glass Door Prism Series)');
    }, 2500);
  };

  // Register appliance API submit
  const submitAppliance = async () => {
    try {
      const res = await fetch(`${BACKEND_URL}/appliances`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          product_category: regCategory,
          model: regModel,
          serial_number: regSerial || null,
          purchase_date: regDate || null
        })
      });
      if (res.ok) {
      ...
      Alert.alert('Network Error', 'Could not register appliance.');
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
        ...
        <View style={{ flex: 1 }}>
          ...
          >
            ...
            <View style={styles.sectionHeaderContainer}>
              <Text style={styles.sectionTitle}>Registered Appliances</Text>
              <TouchableOpacity onPress={() => setRegMode('qr')}>
            ...
            {appliances.length === 0 ? (
              <View style={styles.emptyCard}>
                <Lucide name="box" size={40} color="#555" />
                <Text style={styles.emptyText}>No appliances registered yet.</Text>
                <TouchableOpacity style={styles.miniBtn} onPress={() => setRegMode('manual')}>
                  <Text style={styles.miniBtnText}>Register Manually</Text>
                </TouchableOpacity>
              </View>
            ) : (
                ...
                {appliances.map(app => (
                  <View key={app.id} style={styles.appCard}>
                    <View style={styles.cardHeader}>
                      <Lucide 
                        name={app.product_category.toLowerCase().includes('ref') ? 'thermometer' : 'wind'} 
                        size={24} 
                        color="#007DC5" 
                      />
                    </View>
                    <Text style={styles.appCardTitle}>{app.product_category}</Text>
                    <Text style={styles.appCardModel}>Model: {app.model}</Text>
                    <Text style={styles.appCardServiced}>
                      Serviced: {app.last_serviced ? app.last_serviced.split('T')[0] : 'Never'}
                    </Text>
                    <TouchableOpacity 
                    ...
                    >
                      <Text style={styles.cardActionText}>Troubleshoot</Text>
                      <Lucide name="chevron-right" size={16} color="#007DC5" />
                    </TouchableOpacity>
                  </View>
                ))}
              </View>
      ...
      {/* Appliance Registration Modal / Views */}
      {regMode !== null && (
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>Register Appliance</Text>
              <TouchableOpacity onPress={() => setRegMode(null)}>
              ...
              <View style={styles.scannerWrapper}>
                {qrScanning ? (
                  <View style={styles.cameraBox}>
                    ...
                    <Text style={styles.scannerText}>Simulating camera QR scanner...</Text>
                    {/* Visual overlay scanning lines */}
                    <View style={styles.scannerScanLine} />
                  </View>
                ) : (
                  <View style={{ alignItems: 'center' }}>
                    <TouchableOpacity style={styles.scanBtn} onPress={triggerQrScan}>
                      <Lucide name="camera" size={32} color="#FFF" />
                      <Text style={styles.scanBtnText}>Scan Warranty QR Code</Text>
                    </TouchableOpacity>
                    <TouchableOpacity style={[styles.miniBtn, { marginTop: 20 }]} onPress={() => setRegMode('manual')}>
                      <Text style={styles.miniBtnText}>Register Manually Instead</Text>
                    </TouchableOpacity>
                  </View>
                )}
              </View>
              ...
              <ScrollView style={{ padding: 20 }}>
                ...
                <View style={styles.row}>
                  {['Refrigerator', 'AC', 'Washing Machine', 'Water Dispenser', 'Air Purifier', 'Deep Freezer', 'LED TV', 'Microwave Oven'].map(cat => (
                    <TouchableOpacity
                      key={cat}
                      style={[styles.selectorChip, regCategory === cat && styles.selectorChipActive]}
                      onPress={() => setRegCategory(cat)}
                    >
                      <Text style={[styles.selectorChipText, regCategory === cat && styles.selectorChipTextActive]}>{cat}</Text>
                    </TouchableOpacity>
                  ))}
                </View>
                ...
                <TextInput
                  ...
                  placeholder="e.g. PEL-REF-2026-99"
                  placeholderTextColor="#555"
                />

                <Text style={styles.formLabel}>Purchase Date (Optional)</Text>
                <TextInput
                  style={styles.formInput}
                  value={regDate}
                ...
                <TouchableOpacity style={styles.submitBtn} onPress={submitAppliance}>
                  <Text style={styles.submitBtnText}>Register Appliance</Text>
                </TouchableOpacity>
              </ScrollView>
            )}
          </View>
          ...
          >
            ...
            <View style={styles.chatNavbar}>
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                <TouchableOpacity onPress={() => Linking.openURL('tel:042111200120').catch(() => Alert.alert('Error', 'Dialer not available'))} style={{ marginRight: 15 }}>
                  <Lucide name="phone" size={20} color="#FFF" />
                </TouchableOpacity>
                <TouchableOpacity 
                  onPress={() => {
                    Linking.openURL('whatsapp://send?phone=923111735111').catch(() => {
                ...
                >
                  <Text style={styles.navLinkText}>Talk to human</Text>
                </TouchableOpacity>
              </View>
            </View>
                ...
                <View key={msg.id} style={[styles.bubbleWrapper, msg.role === 'user' ? styles.bubbleUser : styles.bubbleBot]}>
                  <View style={[styles.bubbleContent, msg.role === 'user' ? styles.bubbleContentUser : styles.bubbleContentBot]}>
                    ...
                    {msg.escalate && (
                      ...
                      >
                        <Text style={styles.humanTicketBtnText}>Open Khidmat Markaz App</Text>
                      </TouchableOpacity>
                    )}
                  </View>
                </View>
